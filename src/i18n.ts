...
export const translations = {
  'pt-BR': {
    ...
    vibecodeVsAI: {
      ...
      description: 'Vibecoding é exploração. AI-Assisted Engineering é entrega. Saber distinguir os dois é o primeiro passo.',
      headerAspect: 'Aspecto',
      headerVibe: 'Vibecoding',
      headerAI: 'AI-Assisted Engineering',
      ruleTitle: 'Regra prática',
      ruleText: 'Vibecoding para protótipos e exploração isolada. AI-Assisted Engineering para qualquer entrega que entra no histórico da equipe, toca produção ou é revisada por outra pessoa.',
    ...
    sdd: {
      ...
      description: 'Definir contexto, escopo e critérios antes de executar separa entrega controlada de retrabalho em cascata.',
    ...
    contextSignals: {
      ...
      description: 'Reconhecer esses sinais antes de continuar economiza tempo, reduz retrabalho e evita que bugs de contexto virem bugs de produto.',
      badgeReset: 'Reset',
    ...
    progressiveDisclosure: {
      ...
      workflowCode: 'task → scan index (L1) → select relevant → load instructions (L2) → execute → deep dive on demand (L3)',
    ...
    multiAgent: {
      ...
      description: 'Agente único tem limites reais de contexto e ferramentas. Especialização e paralelismo são a resposta quando a complexidade da tarefa exige.',
      limitsLabel: 'Sinais para escalar',
      limit1Number: 'escopo',
      limit1Label: 'define quando escalar',
      limit1Desc: 'Complexidade crescente de tarefa, domínio ou ferramentas são os sinais reais. Não há threshold absoluto.',
    ...
    antiPatterns: {
      ...
      description: 'A IA acelera erros tanto quanto acelera acertos. Reconhecer esses padrões cedo reduz o custo de correção.',
    ...
    stackCuration: {
      ...
      description: 'Critério de seleção: utilidade prática imediata, baixo risco operacional, documentação pública e curva de adoção compatível com times pequenos.',
  ...
  'en': {
    ...
    hero: {
      ...
      subtitle: 'Technical foundations of AI Coding: context management, controlled execution, and multi-agent architecture.',
    ...
    vibecodeVsAI: {
      ...
      title: 'Vibecoding vs AI-Assisted Engineering',
      description: 'Vibecoding is exploration. AI-Assisted Engineering is delivery. Knowing the difference is the first step.',
      headerAspect: 'Aspect',
      headerVibe: 'Vibecoding',
      headerAI: 'AI-Assisted Engineering',
      ruleTitle: 'Practical rule',
      ruleText: 'Vibecoding for prototypes and isolated exploration. AI-Assisted Engineering for any delivery that enters the team\'s history, touches production, or is reviewed by someone else.',
    ...
    sdd: {
      ...
      description: 'Defining context, scope, and criteria before executing separates controlled delivery from cascading rework.',
      ...
      why2Text: 'The "out of scope" field is mandatory. It is what prevents the AI from expanding the work without an explicit request.',
    ...
    contextSignals: {
      ...
      description: 'Recognizing these signals before continuing saves time, reduces rework, and prevents context bugs from becoming product bugs.',
      badgeReset: 'Reset',
    ...
    progressiveDisclosure: {
      ...
      workflowCode: 'task → scan index (L1) → select relevant → load instructions (L2) → execute → deep dive on demand (L3)',
      workflowNote: 'Each layer is only loaded when the previous one confirms relevance. Context cost is proportional to actual need.',
    },
    layerModel: {
    ...
    executionFlow: {
      ...
      description: 'Each phase has an input, output, and completion criteria. Without a verify gate, the cycle does not close.',
    ...
    multiAgent: {
      ...
      description: 'A single agent has real context and tool limits. Specialization and parallelism apply when the task complexity demands it.',
      limitsLabel: 'Signals to scale',
      limit1Number: 'scope',
      limit1Label: 'signals when to scale',
      limit1Desc: 'Growing task complexity, domain breadth, or tool count are the real signals. There is no absolute threshold.',
    ...
    antiPatterns: {
      ...
      description: 'AI accelerates mistakes as much as it accelerates successes. Recognizing these patterns early reduces the cost of correction.',
    ...
    stackCuration: {
      ...
      description: 'Selection criteria: immediate practical value, low operational risk, public documentation, and adoption curve compatible with small teams.',
...
export const COMPARISONS_DATA: Record<string, Comparison[]> = {
  'pt-BR': [
    { aspect: 'Velocidade inicial', vibecoding: 'Altíssima: gera e aceita na hora', aiAssisted: 'Deliberada: contexto + critério antes de executar', risk: 'ok' },
    { aspect: 'Controle de qualidade', vibecoding: 'Mínimo: confia no output direto', aiAssisted: 'Explícito: revisar, validar, aprovar antes de merge', risk: 'high' },
    { aspect: 'Contexto fornecido', vibecoding: 'Casual: prompt parcial, sem critério', aiAssisted: 'Estruturado: objetivo + escopo + validação definidos', risk: 'high' },
    { aspect: 'Modo de execução', vibecoding: 'Aceita primeiro output, sem loop de verificação', aiAssisted: 'Ciclo RPEV: incrementos curtos com verificação contínua', risk: 'high' },
    { aspect: 'Taxa de retrabalho', vibecoding: 'Alta: problemas aparecem tarde, custo cresce', aiAssisted: 'Baixa: gates precoces eliminam regressão no início', risk: 'high' },
    { aspect: 'Rastreabilidade', vibecoding: 'Baixa: decisões implícitas, diff difícil de revisar', aiAssisted: 'Alta: diff incremental, intenção registrada', risk: 'medium' },
    { aspect: 'Quando usar', vibecoding: 'Protótipos, exploração, prova de conceito isolada', aiAssisted: 'Produção, base de código compartilhada, entrega real', risk: 'ok' },
  ],
  'en': [
    { aspect: 'Initial speed', vibecoding: 'Very high: generates and accepts immediately', aiAssisted: 'Deliberate: context + criteria before executing', risk: 'ok' },
    { aspect: 'Quality control', vibecoding: 'Minimal: trusts direct output', aiAssisted: 'Explicit: review, validate, approve before merge', risk: 'high' },
    { aspect: 'Context provided', vibecoding: 'Casual: partial prompt, no criteria', aiAssisted: 'Structured: objective + scope + validation defined', risk: 'high' },
    { aspect: 'Execution mode', vibecoding: 'Accepts first output, no verification loop', aiAssisted: 'RPEV cycle: short increments with continuous verification', risk: 'high' },
    { aspect: 'Rework rate', vibecoding: 'High: issues surface late, cost grows', aiAssisted: 'Low: early gates eliminate regression at the start', risk: 'high' },
    { aspect: 'Traceability', vibecoding: 'Low: implicit decisions, hard diff to review', aiAssisted: 'High: incremental diff, intent recorded', risk: 'medium' },
    { aspect: 'When to use', vibecoding: 'Prototypes, exploration, isolated proof of concept', aiAssisted: 'Production, shared codebase, real delivery', risk: 'ok' },
  ],
}
...
export const ANTI_PATTERNS_DATA: Record<string, AntiPattern[]> = {
  'pt-BR': [
    { id: 'ap1', category: 'Planejamento', problem: 'Começar direto no código sem contrato técnico', consequence: 'Implementação diverge do objetivo real; retrabalho alto no final.', fix: 'Declarar escopo, critérios de aceite e riscos antes de qualquer execução.', severity: 'critical' },
    { id: 'ap2', category: 'Revisão', problem: 'Commitar output de IA sem revisão humana', consequence: 'Bugs sutis, violações de segurança e regressões entram no histórico.', fix: 'Ler o diff completo antes de todo commit. IA gera hipóteses, não entregas.', severity: 'critical' },
    { id: 'ap3', category: 'Pull Request', problem: 'PR gigante sem gate incremental', consequence: 'Revisão superficial, conflitos de merge e bugs difíceis de isolar.', fix: 'PRs pequenas, com diff legível e aprovação explícita em cada entrega.', severity: 'high' },
  ...
  'en': [
    ...
    { id: 'ap2', category: 'Review', problem: 'Committing AI output without human review', consequence: 'Subtle bugs, security violations, and regressions enter the history.', fix: 'Read the full diff before every commit. AI generates hypotheses, not deliveries.', severity: 'critical' },
    { id: 'ap3', category: 'Pull Request', problem: 'Giant PR without incremental gate', consequence: 'Superficial review, merge conflicts, and bugs hard to isolate.', fix: 'Small PRs with readable diff and explicit approval at each delivery.', severity: 'high' },
...
export const PRINCIPLES_DATA: Record<string, Principle[]> = {
  'pt-BR': [
    { id: 'clarity', title: 'Clareza operacional', subtitle: 'Estado, risco e próxima ação devem ser evidentes em segundos, sem explicação externa.', icon: '◉', signal: 'Signal-first' },
    { id: 'progressive', title: 'Progressive Disclosure', subtitle: 'Carregar índice primeiro. Detalhe só quando necessário. Contexto tem custo.', icon: '◈', signal: 'Context discipline' },
    { id: 'verify', title: 'RPEV + Validate loop', subtitle: 'Output de IA sem verificação contínua é hipótese, não entrega. Gate sempre ativo.', icon: '◍', signal: 'Execution discipline' },
  ],
  'en': [
    { id: 'clarity', title: 'Operational clarity', subtitle: 'State, risk, and next action must be evident in seconds, without external explanation.', icon: '◉', signal: 'Signal-first' },
    { id: 'progressive', title: 'Progressive Disclosure', subtitle: 'Load the index first. Detail only when needed. Context has a cost.', icon: '◈', signal: 'Context discipline' },
    { id: 'verify', title: 'RPEV + Validate loop', subtitle: 'AI output without continuous verification is a hypothesis, not a delivery. Gate always active.', icon: '◍', signal: 'Execution discipline' },
  ],
}

export const PD_LAYERS_DATA: Record<string, ProgressiveLayer[]> = {
  'pt-BR': [
    { id: 'pd1', name: 'Layer 1: Discovery', objective: 'Decisão rápida de relevância com custo mínimo de contexto.', loadWhen: 'Sempre no início da tarefa.', includes: ['Nome da skill / recurso', 'Descrição em uma linha', 'Metadata: tipo, custo, risco'], tokens: '~100 tokens' },
    { id: 'pd2', name: 'Layer 2: Activation', objective: 'Instrução completa apenas para o item selecionado.', loadWhen: 'Somente quando o agente escolhe uma skill / recurso.', includes: ['Regras de execução', 'Parâmetros esperados', 'Ferramentas permitidas'], tokens: '~2k–5k tokens' },
    { id: 'pd3', name: 'Layer 3: Execution', objective: 'Material pesado sob demanda, sem contaminar contexto principal.', loadWhen: 'Somente quando execução exigir.', includes: ['Scripts', 'Exemplos', 'Docs longas', 'Casos de borda'], tokens: 'Zero até ativado' },
  ],
  'en': [
    { id: 'pd1', name: 'Layer 1: Discovery', objective: 'Fast relevance decision with minimal context cost.', loadWhen: 'Always at the start of a task.', includes: ['Skill / resource name', 'One-line description', 'Metadata: type, cost, risk'], tokens: '~100 tokens' },
    { id: 'pd2', name: 'Layer 2: Activation', objective: 'Full instructions only for the selected item.', loadWhen: 'Only when the agent chooses a skill / resource.', includes: ['Execution rules', 'Expected parameters', 'Allowed tools'], tokens: '~2k–5k tokens' },
    { id: 'pd3', name: 'Layer 3: Execution', objective: 'Heavy material on demand, without polluting the main context.', loadWhen: 'Only when execution requires it.', includes: ['Scripts', 'Examples', 'Long docs', 'Edge cases'], tokens: 'Zero until activated' },
  ],
}
...
export const LAYERS_DATA: Record<string, Layer[]> = {
  ...
  'en': [
    ...
    { id: 'project-context', label: 'Project Rules', summary: 'Local contract in AGENTS.md / CLAUDE.md + rules.', chips: ['governance', 'standards'], facts: ['A project contract prevents architecture and style divergence across sessions.', 'Short, objective rules have better agent adherence than long docs.', 'Without a contract, each session reinvents critical decisions inconsistently.'] },
    { id: 'skills', label: 'Skills', summary: 'Reusable, auditable know-how: SKILL.md + scripts/ + references/.', chips: ['workflow', 'reuse'], facts: ['A robust skill clearly defines input, operational limits, and expected output.', 'scripts/ concentrates deterministic execution; references/ loads context on demand.', '$ARGUMENTS and $ARGUMENTS[N] enable dynamic skill parameterization.'] },
    { id: 'mcp', label: 'MCP', summary: 'Access to external data and tools with traceability.', chips: ['integration', 'access'], facts: ['MCP separates "how to do" (skill) from "where to act" (data or tool access).', 'External connections require explicit authentication and a defined permission gate.', 'Without MCP, the agent is a local generator; with MCP, it is an operator with real access.'] },
    { id: 'verification', label: 'Verification', summary: 'Validation gate before considering any delivery complete.', chips: ['check', 'review'], facts: ['Minimum loop: validate → fix → repeat. Never skip on critical code.', 'Destructive actions (push, delete, migration) require explicit human confirmation.', 'CI green + human technical review = necessary condition for completion.'] },
  ],
}
...
export const FLOW_DATA: Record<string, FlowStep[]> = {
  ...
  'en': [
    ...
    { id: 'plan', label: 'Plan', summary: 'Breaks down into verifiable steps with acceptance criteria.', detail: 'Define small steps, each with a clear acceptance criterion and defined fallback. The plan is a technical contract, not a wish list.', payload: 'output: technical plan + criteria per step' },
    { id: 'execute', label: 'Execute', summary: 'Implements in short, traceable increments.', detail: 'Traceable changes to minimize the cost of correction. Review each block before advancing. No bulk commits without review.', payload: 'output: incremental diff + traceability' },
    { id: 'verify', label: 'Verify', summary: 'Validates behavior, quality, and criteria.', detail: 'Run lint, typecheck, build, tests, and risk-oriented review. Without approval evidence, the cycle restarts.', payload: 'gate: evidence-based approval | CI + human review' },
  ],
}
...
export const DEEP_DIVES_DATA: Record<string, DeepDive[]> = {
  ...
  'en': [
    { id: 'dd1', title: 'Skill Architecture', description: 'SKILL.md defines behavior and control frontmatter; scripts/ ensures determinism for heavy logic; references/ extends context without inflating the main prompt.', artifacts: ['SKILL.md', 'scripts/', 'references/', '$ARGUMENTS', '$ARGUMENTS[N]'], links: [{ label: 'Agent Skills Standard', href: 'https://agentskills.io' }, { label: 'anthropics/skills', href: 'https://github.com/anthropics/skills' }] },
    { id: 'dd2', title: 'MCP Integration', description: 'MCP connects the agent to the external world with access control, authentication, and traceability. It transforms the agent from a local generator into an operator with real data and tool access.', artifacts: ['MCP servers', 'auth layer', 'permission gates', 'tool logs'], links: [{ label: 'MCP Intro', href: 'https://modelcontextprotocol.io/docs/getting-started/intro' }, { label: 'Cursor MCP Docs', href: 'https://cursor.com/docs/mcp' }] },
    { id: 'dd3', title: 'Validation Pipeline', description: 'Robust quality pipeline: tests + lint + typecheck + CI + review + fix loop. No delivery is final without explicit approval evidence at every gate.', artifacts: ['CI green', 'AI review', 'human approval', 'merge gate'], links: [{ label: 'Building Effective Agents', href: 'https://www.anthropic.com/engineering/building-effective-agents' }, { label: 'Prompt Engineering', href: 'https://platform.openai.com/docs/guides/prompt-engineering' }] },
    { id: 'dd4', title: '.claude/ Directory Anatomy', description: 'The .claude/ folder is the operational contract between agent and project. Clarity and modularity are inversely proportional to rework and risk.', artifacts: ['CLAUDE.md', 'settings.json', 'rules/', 'skills/', 'agents/', 'hooks/'], links: [{ label: 'Claude Code Docs', href: 'https://docs.anthropic.com/claude-code' }, { label: 'Context7 MCP', href: 'https://context7.com' }] },
  ],
}
...
export const WORKFLOW_PHASES_DATA: Record<string, WorkflowPhase[]> = {
  ...
  'en': [
    ...
    { id: 'wf-setup', step: '1', title: 'Work isolation', objective: 'Create branch/worktree to avoid impacting the baseline.', checks: ['1 feature = 1 branch', 'Descriptive naming', 'Clean environment'], antiPattern: 'Implementing in a shared context without isolation.' },
...
export const STACK_TOOLS_DATA: Record<string, StackTool[]> = {
  'pt-BR': [
    ...
    { name: 'Superpowers', type: 'IDE', why: 'IDE agêntica com suporte a Claude e integração direta a projetos', priority: 'B' },
  ...
  'en': [
    ...
    { name: 'Superpowers', type: 'IDE', why: 'Agentic IDE with Claude support and direct project integration', priority: 'B' },
...
export const MULTI_AGENT_ARCHS_DATA: Record<string, MultiAgentArch[]> = {
  'pt-BR': [
    { id: 'network', letter: 'A', name: 'Network', tagline: 'Agentes comunicam livremente entre si', description: 'Cada agente decide quem é o próximo a executar. Sem coordenador central. Usado em frameworks como Swarm e CrewAI.', when: 'Exploração inicial, protótipos. Não recomendado para produção.', components: ['Agent A', 'Agent B', 'Agent C', 'Peer routing'], tradeoff: 'Pouco confiável e caro: roteamento sem controle gera loops, tokens extras e falhas silenciosas.', reliability: 'low' },
    { id: 'supervisor', letter: 'B', name: 'Supervisor', tagline: 'Agente central delega para especialistas', description: 'Um agente supervisor tem como única função coordenar: ele delega tarefas para subagentes especialistas e agrega os resultados.', when: 'Tarefas compostas com subtarefas bem definidas e especialização por domínio.', components: ['Supervisor Agent', 'Worker: Code', 'Worker: Test', 'Worker: Docs'], tradeoff: 'Controle claro e rastreável. Supervisor vira gargalo se sobrecarregado.', reliability: 'high' },
    { id: 'supervisor-tools', letter: 'C', name: 'Supervisor-as-Tools', tagline: 'Subagentes passados como ferramentas ao LLM central', description: 'Versão simplificada do padrão Supervisor. Os subagentes são registrados como tools, o LLM central os invoca diretamente via tool call.', when: 'Quando o supervisor precisa de baixa latência e o número de workers é fixo e pequeno.', components: ['Orchestrator LLM', 'Worker as Tool A', 'Worker as Tool B', 'Tool registry'], tradeoff: 'Mais simples de implementar; menos flexível para workers dinâmicos.', reliability: 'high' },
    { id: 'hierarchical', letter: 'D', name: 'Hierárquica', tagline: 'Supervisores coordenam outros supervisores', description: 'Camadas de supervisores: um supervisor mestre coordena sub-supervisores, cada um com seu próprio grupo de workers especializados.', when: 'Sistemas grandes com domínios ortogonais: frontend, backend, infra, testes como clusters separados.', components: ['Master Supervisor', 'Sub-supervisor A', 'Sub-supervisor B', 'Workers por cluster'], tradeoff: 'Alta escalabilidade e isolamento de domínio. Complexidade de setup e depuração significativa.', reliability: 'medium' },
    { id: 'custom-cognitive', letter: 'E', name: 'Custom Cognitive', tagline: 'Fluxo personalizado para o domínio específico', description: 'Em vez de usar arquitetura genérica, você constrói o fluxo de controle exato para o seu caso. Maior controle sobre roteamento, estado e ferramentas.', when: 'Produção real com requisitos específicos. Recomendação do LangGraph para casos críticos.', components: ['Domain-specific graph', 'Custom routing', 'State management', 'Typed checkpoints'], tradeoff: 'Mais esforço inicial; menor custo operacional e mais previsibilidade em produção.', reliability: 'high' },
  ],
  'en': [
    { id: 'network', letter: 'A', name: 'Network', tagline: 'Agents communicate freely with each other', description: 'Each agent decides who executes next. No central coordinator. Used in frameworks like Swarm and CrewAI.', when: 'Initial exploration, prototypes. Not recommended for production.', components: ['Agent A', 'Agent B', 'Agent C', 'Peer routing'], tradeoff: 'Unreliable and expensive: uncontrolled routing creates loops, extra tokens, and silent failures.', reliability: 'low' },
    { id: 'supervisor', letter: 'B', name: 'Supervisor', tagline: 'Central agent delegates to specialists', description: 'A supervisor agent has one function: coordinate. It delegates tasks to specialist sub-agents and aggregates results.', when: 'Composite tasks with well-defined subtasks and domain specialization.', components: ['Supervisor Agent', 'Worker: Code', 'Worker: Test', 'Worker: Docs'], tradeoff: 'Clear, traceable control. Supervisor becomes a bottleneck if overloaded.', reliability: 'high' },
    { id: 'supervisor-tools', letter: 'C', name: 'Supervisor-as-Tools', tagline: 'Sub-agents passed as tools to the central LLM', description: 'Simplified version of the Supervisor pattern. Sub-agents are registered as tools; the central LLM invokes them directly via tool call.', when: 'When the supervisor needs low latency and the number of workers is fixed and small.', components: ['Orchestrator LLM', 'Worker as Tool A', 'Worker as Tool B', 'Tool registry'], tradeoff: 'Simpler to implement; less flexible for dynamic workers.', reliability: 'high' },
    { id: 'hierarchical', letter: 'D', name: 'Hierarchical', tagline: 'Supervisors coordinate other supervisors', description: 'Layers of supervisors: a master supervisor coordinates sub-supervisors, each with their own group of specialized workers.', when: 'Large systems with orthogonal domains: frontend, backend, infra, tests as separate clusters.', components: ['Master Supervisor', 'Sub-supervisor A', 'Sub-supervisor B', 'Workers per cluster'], tradeoff: 'High scalability and domain isolation. Significant setup and debugging complexity.', reliability: 'medium' },
    { id: 'custom-cognitive', letter: 'E', name: 'Custom Cognitive', tagline: 'Custom flow for the specific domain', description: 'Instead of a generic architecture, you build the exact control flow for your case. More control over routing, state, and tools.', when: 'Real production with specific requirements. LangGraph is recommended for critical cases.', components: ['Domain-specific graph', 'Custom routing', 'State management', 'Typed checkpoints'], tradeoff: 'More upfront effort; lower operational cost and more predictability in production.', reliability: 'high' },
  ],
}

export const MULTI_AGENT_PATTERNS_DATA: Record<string, MultiAgentPattern[]> = {
  'pt-BR': [
    { id: 'evaluator-optimizer', letter: 'F', name: 'Evaluator-Optimizer', tagline: 'Loop interno de feedback entre gerador e crítico', description: 'Agente gerador produz solução. Agente crítico avalia com critério explícito. Se rejeitado, gerador retrabalha a partir do feedback até aprovação.', when: 'Tarefas criativas ou abertas onde a primeira saída raramente é ideal: código, reviews, docs.', components: ['Generator Agent', 'Critic Agent', 'Feedback loop', 'Acceptance criteria'], tradeoff: 'Mais tokens e latência por ciclo. Saída significativamente melhor para tarefas sem resposta única.' },
    { id: 'quality-loop', letter: 'G', name: 'Quality Loop', tagline: 'Executar, validar, corrigir até aprovação', description: 'Executar a tarefa, validar imediatamente contra critério verificável, corrigir se falhar, repetir. Base do RPEV aplicado em loop automatizado.', when: 'Qualquer tarefa com critério verificável: testes passando, lint limpo, build verde.', components: ['Execute', 'Validate', 'Fix', 'Max retries gate'], tradeoff: 'Simples de implementar. Risco de loop infinito sem limite de retries explícito.' },
    { id: 'dynamic-context', letter: 'H', name: 'Dynamic Context Injection', tagline: 'Shell executa antes do prompt e injeta estado real', description: 'O padrão `!{comando}` executa shell antes da montagem do prompt e injeta saída real no contexto. O agente recebe estado atual, não snapshot estático.', when: 'Quando o agente precisa de estado atual: git status, CI logs, env vars, arquivos de config.', components: ['Shell command', 'Live output capture', 'Prompt assembly', 'Context injection'], tradeoff: 'Risco de prompt injection se a fonte não for confiável. Validar origem antes de injetar.' },
    { id: 'llm-judge', letter: 'I', name: 'LLM-as-a-Judge', tagline: 'Modelo avalia comportamento de outro modelo', description: 'Um LLM separado (ou eval framework como promptfoo) valida a saída do agente principal. Detecta regressão de comportamento sem testes manuais exaustivos.', when: 'Validar respostas abertas, fluxos longos ou comportamento de skills em pipeline de CI.', components: ['Primary agent', 'Judge LLM', 'Eval criteria', 'Pass/fail signal'], tradeoff: 'Custo duplo de tokens. Alto retorno para fluxos críticos onde teste manual não é viável.' },
  ],
  'en': [
    { id: 'evaluator-optimizer', letter: 'F', name: 'Evaluator-Optimizer', tagline: 'Internal feedback loop between generator and critic', description: 'Generator agent produces a solution. Critic agent evaluates with explicit criteria. If rejected, the generator reworks from feedback until approved.', when: 'Creative or open-ended tasks where the first output is rarely ideal: code, reviews, docs.', components: ['Generator Agent', 'Critic Agent', 'Feedback loop', 'Acceptance criteria'], tradeoff: 'More tokens and latency per cycle. Significantly better output for tasks without a single correct answer.' },
    { id: 'quality-loop', letter: 'G', name: 'Quality Loop', tagline: 'Execute, validate, fix until approved', description: 'Execute the task, immediately validate against a verifiable criterion, fix if it fails, repeat. The RPEV foundation applied as an automated loop.', when: 'Any task with a verifiable criterion: tests passing, clean lint, green build.', components: ['Execute', 'Validate', 'Fix', 'Max retries gate'], tradeoff: 'Simple to implement. Risk of infinite loop without an explicit retry limit.' },
    { id: 'dynamic-context', letter: 'H', name: 'Dynamic Context Injection', tagline: 'Shell executes before the prompt and injects live state', description: 'The `!{command}` pattern runs shell commands before prompt assembly and injects live output into context. The agent receives current state, not a static snapshot.', when: 'When the agent needs current state: git status, CI logs, env vars, config files.', components: ['Shell command', 'Live output capture', 'Prompt assembly', 'Context injection'], tradeoff: 'Risk of prompt injection if the source is untrusted. Validate origin before injecting.' },
    { id: 'llm-judge', letter: 'I', name: 'LLM-as-a-Judge', tagline: 'Model evaluates the behavior of another model', description: 'A separate LLM (or eval framework like promptfoo) validates the primary agent output. Detects behavioral regression without exhaustive manual testing.', when: 'Validating open-ended responses, long flows, or skill behavior in a CI pipeline.', components: ['Primary agent', 'Judge LLM', 'Eval criteria', 'Pass/fail signal'], tradeoff: 'Double token cost. High return for critical flows where manual testing is not viable.' },
  ],
}

export const COMM_PATTERNS_DATA: Record<string, CommPattern[]> = {
  'pt-BR': [
    { id: 'shared-state', name: 'Shared State', description: 'Agentes leem e escrevem em um objeto de estado comum. Mensagens, artefatos e contexto ficam acessíveis para todos os participantes do sistema.', pros: ['Visibilidade total do estado', 'Fácil debug e rastreamento', 'Natural para fluxos sequenciais'], cons: ['Estado cresce rapidamente', 'Risco de conflito em escrita paralela', 'Contexto pesado para todos os agentes'] },
    { id: 'tool-calling', name: 'Tool Calling', description: 'Um agente invoca outro como se fosse uma ferramenta: passa parâmetros específicos e recebe apenas o resultado final daquela tarefa.', pros: ['Contexto isolado por tarefa', 'Workers não veem histórico completo', 'Melhor para paralelização'], cons: ['Menos visibilidade do processo interno', 'Difícil propagar contexto parcial', 'Perde histórico intermediário'] },
  ],
  'en': [
    { id: 'shared-state', name: 'Shared State', description: 'Agents read and write to a common state object. Messages, artifacts, and context are accessible to all system participants.', pros: ['Full state visibility', 'Easy debug and tracing', 'Natural for sequential flows'], cons: ['State grows quickly', 'Risk of write conflict in parallel execution', 'Heavy context for all agents'] },
    { id: 'tool-calling', name: 'Tool Calling', description: 'One agent invokes another as if it were a tool: passes specific parameters and receives only the final result of that task.', pros: ['Context isolated per task', 'Workers do not see full history', 'Better for parallelization'], cons: ['Less visibility into the internal process', 'Hard to propagate partial context', 'Loses intermediate history'] },
  ],
}
